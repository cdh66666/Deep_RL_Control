...
- [四足 DRL 运动控制研究](#四足-drl-运动控制研究)
  - [项目日志](#项目日志)
    ...
    - [\[2025 年 3 月 17 日\]](#2025-年-3-月-17-日)
    - [\[2025 年 3 月 18 日\]](#2025-年-3-月-18-日)
    - [\[2025 年 3 月 19 日\]](#2025-年-3-月-19-日)
    - [\[2025 年 3 月 20 日\]](#2025-年-3-月-20-日)
  - [极简环境配置](#极简环境配置)
...
### [2025 年 3 月 2 日]
今天尝试了一下`HIMLoco`项目，花了8个小时1000只狗同时训练了8000轮，效果如下：
![alt text](<img_for_readme/Peek 2025-03-02 08-49.gif>)
训练效果还不错，不过训练时间较长，需要优化。

这几天都在跑开源项目，并没有自己去了解其中源代码，接下来的目标是弄懂legged_gym项目的源代码。
...
**在项目目录中搜索特定关键字**
```bash
grep -r "task_registry.register" .
```
上述命令各部分含义解释如下：
- `-r` 选项：代表递归搜索，这意味着命令会遍历当前目录及其所有子目录进行查找。
- `"task_registry.register"`：为需要搜索的关键字，需用双引号括起来，以确保搜索内容的准确性。
- `.`：表示从当前目录开始搜索。 


### [2025 年 3 月 18 日]
`legged_gym/legged_gym/scripts/train.py`文件第42行，`args.resume = True   #接着上一次的模型继续训练`。
**HIMLoco**项目训练了两个晚上一共9000轮，效果如下：
![alt text](<img_for_readme/Peek 2025-03-19 19-27.gif>)
...
训练一个小时，看看效果:
![alt text](<img_for_readme/Peek 2025-03-20 23-43.gif>)


### [2025 年 3 月 20 日]
撰写论文大纲，[基于深度强化学习的四足机器人运动控制研究-cdh](https://suyvt0crm5.feishu.cn/docx/K0pgdIQWRojkHFxCVRscurLxnqe)
已购买b站isaac gym教程：[华北舵狗王带你5天上手IsaccGym强化学习](https://www.bilibili.com/cheese/play/ep969277?pagetype=payback)
最新资料文档：[狗王5天带你上手Isacc Gym与RL强化学习](https://hcn64ij2s2xr.feishu.cn/wiki/SxkMwYNGxiFMRykizmhc3RBEnFb)
**思考：**
1.用深度学习，学习足端是否脱离地面
2.几千个机器人并行训练原理是什么？
3.当前训练时长：RTX4060：3072个同时训练9000轮-16h可上下坡；如下图；
![alt text](<img_for_readme/Peek 2025-03-19 19-27.gif>)
  期望训练时长：RTX4060：2048个同时训练200轮-20min（一把王者）可上下坡； 
